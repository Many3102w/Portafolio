...
(() => {
  ...
  // Contenedor del orbe 3D (Three.js); muestra el núcleo CSS como respaldo
  // hasta que el orbe carga (o si Three.js no estuviera disponible).
  const ORB = '<span class="jarvis-orb" id="jarvisOrb" aria-hidden="true">' + CORE + '</span>';
  ...
  // ── Orbe 3D de energía (Three.js) en la cabecera ────────────────
  let orbStarted = false;
  async function initOrb() {
    if (orbStarted) return;
    orbStarted = true;
    const host = document.getElementById('jarvisOrb');
    if (!host) return;
    let THREE;
    try {
      THREE = await import('https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.module.js');
    } catch (_) { return; }   // si falla la carga, se queda el núcleo CSS de respaldo

    const w = host.clientWidth || 46, h = host.clientHeight || 46;
    const scene = new THREE.Scene();
    const cam = new THREE.PerspectiveCamera(50, w / h, 0.1, 20);
    cam.position.z = 3.1;
    const renderer = new THREE.WebGLRenderer({ alpha: true, antialias: true });
    renderer.setSize(w, h);
    renderer.setPixelRatio(Math.min(2, window.devicePixelRatio || 1));
    host.innerHTML = '';                 // quita el núcleo CSS de respaldo
    host.appendChild(renderer.domElement);

    // Núcleo: icosaedro en wireframe + esfera interior brillante
    const core = new THREE.Mesh(
      new THREE.IcosahedronGeometry(0.92, 1),
      new THREE.MeshBasicMaterial({ color: 0x1fe0ff, wireframe: true, transparent: true, opacity: 0.9 })
    );
    const inner = new THREE.Mesh(
      new THREE.SphereGeometry(0.42, 24, 24),
      new THREE.MeshBasicMaterial({ color: 0xa9f4ff, transparent: true, opacity: 0.55 })
    );
    scene.add(core, inner);

    // Anillo ecuatorial
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(1.15, 0.025, 8, 48),
      new THREE.MeshBasicMaterial({ color: 0x2f81ff, transparent: true, opacity: 0.85 })
    );
    ...
    for (let i = 0; i < N; i++) {
      const r = 1.25 + Math.random() * 0.55, a = Math.random() * Math.PI * 2, b = Math.acos(2 * Math.random() - 1);
    ...
    const particles = new THREE.Points(pg, new THREE.PointsMaterial({ color: 0x7febff, size: 0.055, transparent: true, opacity: 0.95 }));
    scene.add(particles);
    ...
    (function loop() {
      ...
      const fast = panel.classList.contains('is-speaking') ? 2.6 : 1;   // gira más al hablar
      core.rotation.y += dt * 0.5 * fast;
      core.rotation.x += dt * 0.22 * fast;
      particles.rotation.y -= dt * 0.3 * fast;
      ring.rotation.z += dt * 0.4 * fast;
      const s = 1 + (panel.classList.contains('is-speaking') ? 0.10 * Math.sin(performance.now() * 0.012) : 0);
